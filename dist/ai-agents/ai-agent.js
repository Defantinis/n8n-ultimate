...
export class AIAgent {
     ...
     */
    async suggestSimplifications(workflow, complexNodes, requirements) {
     ...
     */
    buildAnalysisPrompt(requirements) {
...
Keep the workflow practical and implementable.`;
     ...
     */
    buildSimplificationPrompt(workflow, complexNodes, requirements) {
        const nodeDescriptions = complexNodes.map(node => `${node.name} (${node.type}): ${Object.keys(node.parameters).length} parameters`).join('\n');
...
Focus on maintaining functionality while reducing complexity.`;
    ...
    async callOllama(prompt) {
        ...
        if (this.enableCaching) {
            const cachedResponse = await ollamaCacheManager.getCachedResponse(prompt, this.modelName, temperature, topP, numPredict);
            if (cachedResponse) {
        ...
        try {
            ...
            if (this.enableCaching) {
                ...
                // Cache the response
                await ollamaCacheManager.setCachedResponse(prompt, data.response, this.modelName, temperature, topP, numPredict);
            }
            ...
            if (jsonMatches && jsonMatches.length > 0) {
                ...
                if (parsed.nodes && parsed.flow) {
                    return {
                        ...
                        flow: parsed.flow.map((connection) => ({
     ...
     */
    parseSimplificationResponse(response, complexNodes) {
        try {
     ...
     */
    createFallbackAnalysis(requirements) {
     ...
     */
    createFallbackSimplifications(complexNodes) {
        return complexNodes.map(node => ({
            type: 'simplify-parameters',
            nodeId: node.id,
    ...
    determineWorkflowType(requirements) {
        ...
        if (requirements.description.toLowerCase().includes('if') ||
            requirements.description.toLowerCase().includes('condition'))
            return 'conditional';
        return 'linear';
    }
    estimateComplexity(requirements) {
        let complexity = 3; // Base complexity
        if (requirements.inputs && requirements.inputs.length > 2)
            complexity += 1;
        if (requirements.outputs && requirements.outputs.length > 2)
            complexity += 1;
        if (requirements.steps && requirements.steps.length > 5)
            complexity += 2;
        if (requirements.constraints?.maxNodes && requirements.constraints.maxNodes > 10)
            complexity += 2;
        return Math.min(10, complexity);
    }
    extractKeyComponents(requirements) {
        const components = [];
        if (requirements.inputs?.some(i => i.type === 'webhook'))
            components.push('webhook');
        if (requirements.inputs?.some(i => i.type === 'api'))
            components.push('HTTP');
        if (requirements.outputs?.some(o => o.type === 'file'))
            components.push('file-processing');
        if (requirements.outputs?.some(o => o.type === 'email'))
            components.push('email');
        if (requirements.type === 'data-processing')
            components.push('data-processing');
        return components;
    }
    suggestNodeTypes(requirements) {
        const nodeTypes = ['n8n-nodes-base.start'];
        if (requirements.inputs?.some(i => i.type === 'webhook')) {
