...
export class ConnectionBuilder {
     ...
     */
    addConnection(connections, fromNode, toNode, flowConnection) {
     ...
     */
    determineConnectionType(flowConnection) {
     ...
     */
    determineOutputIndex(fromNode, flowConnection) {
     ...
     */
    determineInputIndex(toNode, _flowConnection) {
     ...
     */
    buildParallelConnections(triggerNode, parallelNodes, mergeNode) {
        const connections = {};
     ...
     */
    buildConditionalConnections(conditionNode, truePathNodes, falsePathNodes, mergeNode) {
        const connections = {};
        // Initialize connections
     ...
     */
    addConnectionByIds(connections, fromNodeName, toNodeName, type = 'main', outputIndex = 0, inputIndex = 0) {
     ...
     */
    rerouteThroughNode(connections, targetNodeName, intermediateNodeName) {
        let newConnections = JSON.parse(JSON.stringify(connections));
        let originalSuccessorName = null;
        if (newConnections[targetNodeName]?.main?.[0]?.[0]) {
            ...
            newConnections = this.addConnectionByIds(newConnections, targetNodeName, intermediateNodeName);
        }
