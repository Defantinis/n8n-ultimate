...
export class PositionCalculator {
     ...
     */
    positionLinearWorkflow(nodes, structure) {
     ...
     */
    positionParallelWorkflow(nodes, structure, _connections) {
        let currentX = this.START_X;
     ...
     */
    positionConditionalWorkflow(nodes, structure, _connections) {
        let currentX = this.START_X;
        for (let layerIndex = 0; layerIndex < structure.layers.length; layerIndex++) {
            const layer = structure.layers[layerIndex];
            if (layerIndex === 0) {
     ...
     */
    positionComplexWorkflow(nodes, connections) {
    ...
    findEndNodes(nodes, connections) {
        // End nodes are those that don't have any outgoing connections
        return nodes.filter(node => {
            const nodeConnections = connections[node.name];
            if (!nodeConnections)
                return true;
            return Object.values(nodeConnections).every(outputs => outputs.every(outputConnections => outputConnections.length === 0));
        });
    }
    isLinearStructure(connections) {
    ...
    isParallelStructure(connections) {
    ...
    isConditionalStructure(nodes, _connections) {
        // Check for IF nodes or similar conditional nodes
        return nodes.some(node => node.type === 'n8n-nodes-base.if' ||
            node.type === 'n8n-nodes-base.switch');
    }
    createLinearLayers(nodes, connections) {
        const layers = [];
    ...
    createParallelLayers(nodes, connections) {
        // Similar to linear but handles parallel branches
        return this.createLinearLayers(nodes, connections);
    }
    createConditionalLayers(nodes, connections) {
        // Handle conditional branching
        return this.createLinearLayers(nodes, connections);
    }
    createComplexLayers(nodes, connections) {
