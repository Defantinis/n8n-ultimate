...
export class MemoryManagerTestData {
     ...
     */
    static generateMockMemoryStats(overrides) {
        return {
     ...
     */
    static generateStressTestObjects(count) {
        const objects = [];
     ...
     */
    static generateWorkflowScenarios() {
        return [
            {
     ...
     */
    static generatePoolConfigurations() {
        return [
            {
                name: 'small-objects',
                maxSize: 50,
...
export class MemoryManagerTest {
     ...
     */
    async testMemoryStatsCollection() {
     ...
     */
    async testWorkflowContextManagement() {
     ...
     */
    async testMemoryPoolOperations() {
        const testName = 'Memory Pool Operations';
        const startTime = Date.now();
        try {
            const poolName = 'test-pool';
            // Create memory pool
            this.memoryManager.createMemoryPool(poolName, () => ({ data: Buffer.alloc(1024) }), (obj) => obj.data.fill(0), 10);
     ...
     */
    async testGarbageCollectionTrigger() {
     ...
     */
    async testPoolUtilizationOptimization() {
        ...
        try {
            ...
            this.memoryManager.createMemoryPool(poolName, () => ({ id: Math.random(), data: new Array(100).fill(0) }), (obj) => {
                obj.id = 0;
                obj.data.fill(0);
            }, 3 // Small pool size
            );
    ...
    async testConcurrentWorkflowHandling() {
        ...
        try {
            ...
            const cleanupPromises = workflowIds.map(workflowId => this.memoryManager.cleanupWorkflowContext(workflowId));
     ...
     */
    async testMemoryPoolPerformance() {
        ...
        try {
            ...
            this.memoryManager.createMemoryPool(poolName, () => ({ data: new Array(100).fill(Math.random()) }), (obj) => obj.data.fill(0), 100);
     ...
     */
    async testLargeWorkflowMemoryUsage() {
     ...
     */
    async testMemoryCleanupEfficiency() {
     ...
     */
    async testMemoryThresholdBoundaries() {
     ...
     * Record test result
     */
    recordTestResult(test, passed, message, duration) {
...
//# sourceMappingURL=test-memory-manager.js.map