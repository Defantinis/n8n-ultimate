...
export class OllamaCacheManagerTest {
    ...
    async testCacheKeyGeneration() {
        ...
        for (const testPrompt of testPrompts) {
            await this.cacheManager.setCachedResponse(testPrompt.prompt, testPrompt.expectedResponse, testPrompt.model, testPrompt.temperature, testPrompt.topP, testPrompt.numPredict);
            const cachedResponse = await this.cacheManager.getCachedResponse(testPrompt.prompt, testPrompt.model, testPrompt.temperature, testPrompt.topP, testPrompt.numPredict);
            if (!cachedResponse) {
            ...
            const missResponse = await this.cacheManager.getCachedResponse(testPrompt.prompt, testPrompt.model, testPrompt.temperature + 0.1, // Different temperature
            testPrompt.topP, testPrompt.numPredict);
            if (missResponse) {
    ...
    async testCacheEviction() {
        ...
        for (let i = 0; i < maxEntries + 10; i++) {
            await this.cacheManager.setCachedResponse(`Test prompt ${i}`, `Test response ${i}`, "llama3.2");
        }
    ...
    async testMemoryManagement() {
        ...
        for (let i = 0; i < 10; i++) {
            await this.cacheManager.setCachedResponse(`Memory test prompt ${i}`, `Memory test response ${i}`.repeat(100), // Larger response
            "llama3.2");
        }
        const stats = this.cacheManager.getCacheStats();
    ...
    async testConcurrentAccess() {
        ...
        // Concurrent writes
        for (let i = 0; i < 10; i++) {
            promises.push(this.cacheManager.setCachedResponse(`concurrent ${i}`, `response ${i}`, "llama3.2"));
        }
        await Promise.all(promises);
        // Concurrent reads
        const readPromises = [];
        for (let i = 0; i < 10; i++) {
            readPromises.push(this.cacheManager.getCachedResponse(`concurrent ${i}`, "llama3.2"));
        }
 ...
 * Main test execution
 */
export async function testOllamaCacheManager() {
...
//# sourceMappingURL=test-ollama-cache-manager.js.map