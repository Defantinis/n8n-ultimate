 ...
 */
import { N8nIntegrationManager, N8nTestUtils, ConsoleLogger, DEFAULT_INTEGRATION_CONFIG } from './integration-patterns.js';
/**
...
export class IntegrationPatternsTestSuite {
    ...
    async testPerformanceOptimizer() {
        ...
        const batchResults = await optimizer.batchProcess(items, async (item) => item * 2, 2);
        if (batchResults.length !== 5 || batchResults[0] !== 2) {
            throw new Error('Batch processing failed');
        }
        // Test parallel processing
        const parallelResults = await optimizer.parallelProcess(items, async (item) => {
    ...
    async testWebhookHandler() {
        ...
        const isValid = webhookHandler.validateWebhookSignature(payload, `sha256=${signature}`, secret);
        if (!isValid) {
     ...
     */
    async testMockWorkflowCreation() {
    ...
    async runPerformanceBenchmark() {
        ...
        const slowOperation = () => new Promise(resolve => setTimeout(() => resolve(Math.random()), 10));
        // Without cache
 ...
 */
export async function runIntegrationPatternsTests() {
...
//# sourceMappingURL=test-integration-patterns.js.map