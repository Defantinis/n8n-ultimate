...
import { ErrorSeverity, ErrorCategory, ErrorType, RecoveryStrategy } from './error-classifier';
/**
...
(function (ReportFormat) {
    ...
    ReportFormat["PLAIN_TEXT"] = "plain_text";
...
(function (DetailLevel) {
    ...
    DetailLevel["TECHNICAL"] = "technical"; // All technical information for developers
...
(function (SuggestionType) {
    SuggestionType["USER_ACTION"] = "user_action";
    SuggestionType["SYSTEM_CHECK"] = "system_check";
    SuggestionType["CONFIGURATION"] = "configuration";
    SuggestionType["TROUBLESHOOTING"] = "troubleshooting";
    SuggestionType["CONTACT_SUPPORT"] = "contact_support";
    SuggestionType["WORKAROUND"] = "workaround"; // Alternative approaches
...
export class ErrorReporter extends EventEmitter {
     ...
     */
    generateReport(error, context, options = {}) {
     ...
     */
    formatReport(report, format = this.config.defaultFormat) {
        switch (format) {
     ...
     */
    getSuggestions(errorType, context, maxSuggestions = this.config.maxSuggestions) {
    ...
    getReports(criteria) {
        ...
        if (criteria.timeRange) {
            reports = reports.filter(r => r.timestamp >= criteria.timeRange.start &&
                r.timestamp <= criteria.timeRange.end);
        }
        if (criteria.resolved !== undefined) {
            reports = reports.filter(r => criteria.resolved ? r.metadata.resolution?.status === 'resolved' :
     ...
     */
    markResolved(reportId, resolvedBy, resolution) {
     ...
     */
    createErrorReport(classifiedError, options) {
...
<div class="error-report ${severityClass}">
  ...
  <div class="error-meta">
    ...
    generateTitle(classifiedError, template) {
        if (template) {
    ...
    generateDescription(classifiedError, template) {
        if (template) {
    ...
    generateSecondaryMessage(classifiedError) {
        const messages = {
    ...
    generateImpactDescription(classifiedError) {
        const impacts = {
    ...
    getComponentFromCategory(category) {
        const components = {
    ...
    generateDocumentationUrl(errorType) {
