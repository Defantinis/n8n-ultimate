...
export class ErrorReporterTest {
    ...
    setupEventListeners() {
        this.reporter.on('reportGenerated', (report) => {
            console.log(`📋 Report generated: ${report.id} (${report.summary.severity})`);
        });
        this.reporter.on('reportResolved', (report) => {
            console.log(`✅ Report resolved: ${report.id}`);
        });
        this.reporter.on('criticalErrorReport', (report) => {
    ...
    testCommunityNodeErrorReporting() {
        try {
...
//# sourceMappingURL=test-error-reporter.js.map