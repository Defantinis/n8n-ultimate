...
export class PerformanceValidator {
    connectionValidator;
    thresholds;
    constructor(connectionValidator, customThresholds) {
     ...
     */
    generatePerformanceReport(workflow) {
     ...
     */
    validateNodePerformance(workflow) {
        const errors = [];
        const warnings = [];
        const nodeMetrics = this.analyzeNodePerformance(workflow);
     ...
     */
    validateResourceUsage(workflow) {
        const errors = [];
        const warnings = [];
     ...
     */
    analyzeNodePerformance(workflow) {
     ...
     */
    analyzeWorkflowPerformance(workflow, nodeMetrics) {
    ...
    estimateDataVolumeMultiplier(node) {
    ...
    calculateOptimizationPotential(perfData, complexity) {
    ...
    generateNodeWarnings(node, perfData, executionTime, memoryUsage) {
        const warnings = [];
    ...
    calculateCriticalPath(workflow, nodeMetrics) {
    ...
    calculateTotalExecutionTime(criticalPath, nodeMetrics) {
        return criticalPath.reduce((total, nodeId) => {
            const metric = nodeMetrics.find(m => m.nodeId === nodeId);
            return total + (metric?.estimatedExecutionTime || 0);
        }, 0);
    }
    calculateMaxConcurrentNodes(workflow) {
    ...
    calculateNodeDepth(workflow, nodeId) {
    ...
    calculateScalabilityScore(workflow, nodeMetrics) {
    ...
    identifyBottlenecks(nodeMetrics) {
        return nodeMetrics.filter(m => m.bottleneckRisk > 0.7).map(m => m.nodeId);
    }
    identifyOptimizationOpportunities(workflow, nodeMetrics) {
        const opportunities = [];
    ...
    calculateOverallScore(workflowMetrics, nodeMetrics) {
    ...
    checkThresholds(workflowMetrics) {
    ...
    countOptimizationOpportunities(nodeMetrics) {
        return nodeMetrics.reduce((acc, m) => acc + m.recommendations.length, 0);
    }
    generateRecommendations(workflowMetrics, nodeMetrics, validationResult) {
