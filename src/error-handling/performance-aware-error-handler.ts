...
import { ErrorClassifier, ClassifiedError, ErrorContext } from './error-classifier';
import { AdvancedErrorLogger, TelemetryData } from './error-logger';
import { PerformanceErrorMonitor, ErrorHandlingMetrics } from './performance-error-monitor';
import { 
  AdaptiveErrorCollector, 
  CollectionMode, 
  ProcessingResult,
  CollectionStats 
} from './adaptive-error-collector';
...
export interface PerformanceAwareConfig {
  ...
  performanceThresholds: {
    cpu: { optimal: number; degraded: number; critical: number };
    memory: { optimal: number; degraded: number; critical: number };
    eventLoopLag: { optimal: number; degraded: number; critical: number };
    errorHandlingOverhead: { optimal: number; degraded: number; critical: number };
  };
...
export class PerformanceAwareErrorHandler extends EventEmitter {
  ...
  private errorLogger: AdvancedErrorLogger;
  private performanceMonitor: PerformanceErrorMonitor;
  private adaptiveCollector: AdaptiveErrorCollector;
  private config: PerformanceAwareConfig;
  ...
  public async handleError(
  ...
  ): Promise<PerformanceAwareResult> {
    ...
    try {
      ...
      const result: PerformanceAwareResult = {
  ...
  private initialize(): void {
    ...
    this.performanceMonitor = new PerformanceErrorMonitor(
    ...
    this.adaptiveCollector.setMode(
      this.config.adaptiveCollection.initialMode,
   ...
   * Calculate CPU delta
   */
  private calculateCpuDelta(): number {
   ...
   */
  private updateErrorStats(processingTime: number, success: boolean): void {
    if (success) {
  ...
  private determineOverallHealth(
    ...
    if (
      performance.performanceStatus === 'critical' ||
    ...
    if (
      performance.performanceStatus === 'degraded' ||
  ...
  private generateHealthRecommendations(
    ...
    collection: CollectionStats,
    errorHandling: any
  ): SystemHealthReport['recommendations'] {
   ...
   */
  private getActiveAlerts(metrics: ErrorHandlingMetrics): SystemHealthReport['activeAlerts'] {
    const alerts: SystemHealthReport['activeAlerts'] = [];
   ...
   */
  private calculatePerformanceTrends(history: ErrorHandlingMetrics[]) {
    if (history.length < 2) {
      return {
        cpuTrend: 'stable' as const,
        memoryTrend: 'stable' as const,
        overheadTrend: 'stable' as const
      };
    }
  ...
  private calculateExpectedImprovement(
    currentMetrics: ErrorHandlingMetrics,
    newMode: CollectionMode
  ): string {
    const reductions = {
   ...
   */
  private async fallbackErrorHandling(error: Error, context: ErrorContext): Promise<void> {
    try {
