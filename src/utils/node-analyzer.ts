...
export class NodeAnalyzer {
  ...
  calculateNodeComplexity(node: N8nNode): number {
    ...
    if (paramCount > 5) complexity += 1;
    if (paramCount > 10) complexity += 1;

    // Add complexity for custom code
    if (this.hasCustomCode(node)) {
      const codeLength = this.getCodeLength(node);
      if (codeLength > 100) complexity += 1;
      if (codeLength > 500) complexity += 1;
    }

    // Add complexity for expressions
    const expressionCount = this.countExpressions(node);
    if (expressionCount > 2) complexity += 1;
    if (expressionCount > 5) complexity += 1;
   ...
   */
  private getCodeLength(node: N8nNode): number {
   ...
   */
  private countExpressions(node: N8nNode): number {
    let count = 0;
   ...
   */
  private calculateConnectionStrength(workflow: N8nWorkflow, sourceNode: string, targetNode: string): number {
    // Base strength
    let strength = 1;
    ...
    if (!source || !target) return strength;
...
export interface NodeRelationshipAnalysis {
  ...
  criticalNodes: Array<{ nodeName: string; influence: number }>;
