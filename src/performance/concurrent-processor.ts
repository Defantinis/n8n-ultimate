...
class TaskQueue extends EventEmitter {
    ...
    private getNextAvailableTask(queue: Task[]): Task | null {
    ...
    public markTaskCompleted(taskId: string, result: TaskResult): void {
...
class WorkerPool extends EventEmitter {
    ...
    public getAvailableWorker(): number | null {
        return this.availableWorkers.shift() || null;
    }

    public assignTask(workerId: number, task: Task): void {
...
export class ConcurrentProcessor extends EventEmitter {
    private config: ConcurrentProcessorConfig;
    private taskQueue: TaskQueue;
    private workerPool: WorkerPool;
    private parallelExecutor: ParallelExecutionManager;
    ...
    constructor(config: Partial<ConcurrentProcessorConfig> = {}) {
        ...
        const defaultConfig: ConcurrentProcessorConfig = {
    ...
    public async addTask(task: Omit<Task, 'id' | 'timestamp'>): Promise<string> {
        const fullTask: Task = {
    ...
    public async addBatch(tasks: Array<Omit<Task, 'id' | 'timestamp'>>): Promise<string[]> {
    ...
    public async executeParallel<T>(
        tasks: Array<() => Promise<T>>,
        options: { batchSize?: number; concurrencyLimit?: number } = {}
    ): Promise<Array<{ success: boolean; result?: T; error?: string }>> {
...
if (!isMainThread && workerData?.isWorker) {
    ...
    async function computeTask(payload: any): Promise<any> {
    ...
    async function ioTask(payload: any): Promise<any> {
        // Simulate I/O work
        await new Promise(resolve => setTimeout(resolve, payload.delay || 50));
        return { processed: payload.data };
    }

    async function transformTask(payload: any): Promise<any> {
