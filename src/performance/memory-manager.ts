...
export interface MemoryThresholds {
  warningThreshold: number; // Percentage of heap usage to trigger warning
  criticalThreshold: number; // Percentage to trigger aggressive cleanup
  maxHeapSize: number; // Maximum heap size in bytes
  gcTriggerThreshold: number; // Threshold to trigger manual GC
...
export class MemoryManager extends EventEmitter {
  ...
  private activeContexts: Map<string, WorkflowMemoryContext> = new Map();
  ...
  private heapSnapshots: Array<{ timestamp: number; snapshot: any }> = [];
  ...
  createWorkflowContext(workflowId: string): WorkflowMemoryContext {
    ...
    const context: WorkflowMemoryContext = {
      workflowId,
      startMemory,
   ...
   */
  async cleanupWorkflowContext(workflowId: string): Promise<void> {
    const context = this.activeContexts.get(workflowId);
    if (!context) return;
  ...
  takeHeapSnapshot(): void {
    if (!this.config.enableHeapSnapshots) return;
   ...
   */
  private createDefaultMemoryPools(): void {
    if (!this.config.enableMemoryPools) return;

    // Node object pool
    this.createMemoryPool(
      'nodes',
      () => ({}),
      (node) => {
   ...
   */
  private assessMemoryLeakRisk(): 'low' | 'medium' | 'high' {
    if (this.memoryHistory.length < 5) return 'low';
  ...
  async emergencyCleanup(): Promise<void> {
    ...
    const cleanupPromises = Array.from(this.activeContexts.keys()).map(
      workflowId => this.cleanupWorkflowContext(workflowId)
    );
