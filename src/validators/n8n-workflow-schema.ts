...
// Core workflow structure interfaces
...
// Node schema with validation rules
...
// Connections schema - actual n8n format
...
// Static data for persistent workflow state
...
// Pin data for testing and debugging
...
export const N8N_VALIDATION_RULES = {
  ...
  REQUIRED_WORKFLOW_FIELDS: ['id', 'name', 'nodes', 'connections'] as const,
  REQUIRED_NODE_FIELDS: ['id', 'name', 'type', 'typeVersion', 'position', 'parameters'] as const,
  ...
  CONSTRAINTS: {
    ...
    CONNECTION_TYPES: ['main', 'error'] as const,
...
export class N8nWorkflowSchemaValidator {
   ...
   * Validate required fields
   */
  private static validateRequiredFields(workflow: any, errors: ValidationError[]): void {
   ...
   * Validate workflow structure
   */
  private static validateWorkflowStructure(workflow: any, errors: ValidationError[], warnings: ValidationError[]): void {
   ...
   */
  private static validateNodes(nodes: any[], errors: ValidationError[], warnings: ValidationError[]): void {
   ...
   * Validate connections
   */
  private static validateConnections(connections: any, nodes: any[], errors: ValidationError[], warnings: ValidationError[]): void {
    ...
    Object.entries(connections).forEach(([sourceNodeId, nodeConnections]) => {
      ...
      Object.entries(nodeConnections as any).forEach(([connectionType, connectionList]) => {
        // Validate connection type
        if (!N8N_VALIDATION_RULES.CONSTRAINTS.CONNECTION_TYPES.includes(connectionType as any)) {
          warnings.push({
        ...
        (connectionList as any[]).forEach((connectionGroup, groupIndex) => {
        ...
        if ((connectionList as any[]).length > N8N_VALIDATION_RULES.CONSTRAINTS.MAX_CONNECTIONS_PER_OUTPUT) {
          warnings.push({
            field: `connections.${sourceNodeId}.${connectionType}`,
            message: `High number of connection groups (${(connectionList as any[]).length}) may impact performance`,
   ...
   */
  private static generateSuggestions(workflow: any, suggestions: string[]): void {
    // Suggest adding error handling
    const nodesWithoutErrorHandling = (workflow.nodes || []).filter((node: any) => 
      !node.onError && node.type !== 'n8n-nodes-base.start'
    );
   ...
   */
  private static calculateCompatibilityScore(errors: ValidationError[], warnings: ValidationError[]): number {
