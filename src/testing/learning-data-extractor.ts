...
`;
  ...
  static analyzeLearningData(learningDataJson: string): {
    ...
    try {
      const sessions: LearningSession[] = JSON.parse(learningDataJson);
  ...
  private static countPatterns(values: string[]): Record<string, number> {
  ...
  private static extractCommonIssues(sessions: LearningSession[]): any[] {
  ...
  private static analyzeImprovementTrends(sessions: LearningSession[]): any {
  ...
  private static generateRecommendations(summary: any, patterns: any): string[] {
